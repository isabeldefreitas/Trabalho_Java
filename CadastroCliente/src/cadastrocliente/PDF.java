...
import cadastrocliente.Venda;
import cadastrocliente.ClientesBD;
...
public void gerarPdf(String codigo) {
        ...
        String arq = "C:\\pdfs\\PDF_" + codigo + ".pdf";
        try {
            PdfWriter.getInstance(document, new FileOutputStream(arq));
            document.open();
           document.add(new Paragraph(Lista.get(0).getData()));
           document.add(new Paragraph("Venda nº " + codigo ));
