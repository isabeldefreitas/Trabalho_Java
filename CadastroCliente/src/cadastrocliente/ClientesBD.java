...
public class ClientesBD {
    ...
    public void adicionarCliente(Cliente c){
        String insert = "insert into vendapdv.cliente(codigo, nome, telefone, email, rua, casa, complemento) values(?,?,?,?,?,?,?)";
        editarBanco(insert, c, 1);
        JOptionPane.showMessageDialog(null, "Cliente adicionado!");
    }
    
    public void alterarCliente(Cliente c){
        String update = "update vendapdv.cliente set codigo = ?, nome = ?, telefone = ?, email = ?, rua = ?, casa = ?, complemento = ? where codigo = ?";
    ...
    public void excluirCliente(String codigo){
        try {
            String delete = "delete from vendapdv.cliente where codigo = ?";
    ...
    public ArrayList<Cliente>lerBancoClientes(){
        try {
            ...
            String select = "select * from vendapdv.cliente";
    ...
    public String buscaNome(String codigo){
        try {
            String nome = null;
            String select = "select nome FROM vendapdv.cliente WHERE codigo = ?";
            PreparedStatement stmt = conexao.prepareStatement(select);
            stmt.setString(1, codigo);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
